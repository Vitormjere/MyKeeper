...
async function buscar(id){
    const retorno = await fetch('/mykeeper/src/Controllers/tickets_suporte_get.php?id='+id);
...
async function alterar(){
    ...
    const retorno = await fetch('/mykeeper/src/Controllers/tickets_suporte_alterar_back.php?id='+id, {
        method: 'POST',
        body: fd
    });
    ...
    if(resposta.status == 'ok'){
        ...
        setTimeout(() => {
        window.location.href = "/mykeeper/tickets_suporte";}, 1000);
    }else{
