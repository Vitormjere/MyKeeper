...
function preencherTabela(tabela) {
    ...
    for (var i = 0; i < tabela.length; i++) {
        const icone = tabela[i].icone
            ? `<img src="${e(normalizarCaminhoArquivo(tabela[i].icone))}" style="width:40px; height:40px;">`
            : 'Sem ícone';
...
async function excluir(id) {
    const retorno = await fetch('../Controllers/categoria_excluir.php?id=' + id);
    const resposta = await retorno.json();
    if (resposta.status == 'ok') {
...
document.getElementById('categoria_nova').addEventListener('click', () => {
    window.location.href = 'categoria_novo.php';
});
